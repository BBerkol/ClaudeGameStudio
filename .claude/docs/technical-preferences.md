...
- **ADR-0013** (Accepted 2026-06-13) — Run-Scoped Card Collection + Reward-Source Composition: `RunDeck` POCO in `WastelandRun.Run` (distinct from `Combat.Deck` shuffle engine); `CombatReward` extends additively to `(int Scrap, CardOffer Choices)`; sibling `ICardRewardSource` seam (not folded into `IRewardSource`); `Milestone1Starter` hardcoded factory on `RunDeck`; `PendingCardOffer` on `RunState` per ADR-0004; dupe-filter seam at the source; `CardOffer.OfferSeed` recorded for M2 pity; salt `0x4341` ('CA') for card-offer derivation; `StarterDecks.MakeBasic()` retired (ADR-0011 compliance)
- **ADR-0018** (Accepted 2026-09-13) — UGUI Retention for Transient World-Anchored Annotations: **supersedes ADR-0014**, whose central premise (retained `Popups` canvas is world-space) is FALSE — it is `m_RenderMode: 1` ScreenSpaceCamera and `CombatHud.ApplyScreenSpaceCamera` forces it there every build. A 2026-09-13 audit of all 14 shipping canvases disproved five further claims, including a performance rationale that is self-refuting for its own example. UI Toolkit remains primary. UGUI is retained for one *category*: transient, per-event annotations positioned by a live world transform with no persistent screen-space layout. Membership is a closed **registry** — `Popups`, `HitZonesCanvas`, `IntentCanvas`, `TargetingReadoutCanvas` — each with a written exit criterion, capped at 5 members before the category must be re-derived. Deliberately NOT a grep predicate: both candidates fail in opposite directions (render-mode excludes `Popups` and catches inert-nested `CardHand`; vehicle-parentage catches `HudAnchors`, the largest P4 surface). P5 rescoped from pattern match to registry check. Routing: `unity-ui-specialist`
- **ADR-0014** (**Superseded 2026-09-13 by ADR-0018** — body retained unedited for P3's landed detail and as the record of what was believed; do not cite as current policy) — UI Toolkit as Primary Stack, UGUI for World-Space Popups Only: UI Toolkit (UXML+USS+C# controller) is primary for all screens/menus/HUD/map/picker/overlay; UGUI retained only for `Popups` canvas (world-space damage numbers/status popups); axis-aligned hybrid (world-space vs screen-space) ≠ bimodal per ADR-0011; no `UnityEvent` under new stack (ports ADR-0002 rule forward); 5-phase migration (P1 design tokens before Slice 6 → P2 Slice 6 native → P3 picker+overlay → P4 Combat_HUD M1.5 → P5 CI grep gate); `WastelandRun.UI` asmdef one-way arrow to Run+Combat. Routing: `unity-ui-specialist`
- **ADR-0015** (Accepted 2026-06-15) — Biome Distribution as Configuration Narrowing: codifies ADR-0011 exception #4 (polymorphism via data tables) as the canonical scope-narrowing pattern. Full `BeaconType` enum + canonical `BiomeWebGenerator` + narrow `BiomeDistributionSO` data table = ADR-0011-compliant scope narrowing without stubs / bimodal paths / transitional comments. First application: Slice 6 ships `BiomeDistributionSO` + `BiomeWebGenerator` + `Biome1Distribution.asset` (Combat + Haven only); future beacon-type handlers (Merchant, Event, Rest, Chopshop, EliteCombat) ship as canonical vertical slices. Pattern is **content-shaped** (generation, selection, sampling) — behavioral seams use ADR-0013 composition instead.
