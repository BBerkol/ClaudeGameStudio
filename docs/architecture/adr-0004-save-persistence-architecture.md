...
Resume-atomic groups are declared in this ADR. **Membership criterion (Slice 8d Amendment 2026-06-25):**

> A DTO joins an existing resume-atomic group if its absence-with-other-group-members-present creates a silently-broken determinism or progression invariant — i.e., the run would resume to a state that is mechanically incoherent in a way the player cannot be meaningfully warned about. Independently recoverable state with a surfaceable default (scrap = 0, status = Ongoing) does **not** qualify and stays standalone.

**Resume-atomic groups (current):**

| Group | Members | Reason |
|-------|---------|--------|
| `run.session_core` (renamed from `run.seed_map` at Slice 8d 2026-06-25) | `run.run_seed`, `run.node_map`, `run.run_deck` | `NodeMap.MapSeed = RunSeed ^ 0x4D41` per ADR-0003. NodeMap rehydrated against a regenerated RunSeed would carry a `MapSeed` whose provenance no longer matches its own structure — every per-step derivation (`RunSeed ^ stepIndex ^ <salt>` per ADR-0003 Rule 3 / RunController) downstream would silently diverge from the no-crash counterfactual. Players would not notice individual rolls, but a run that crashes through a pity-counter boundary would silently desync. Both-or-neither is the only policy that preserves the **ADR-0003 `RunSeed ^ stepIndex` contract across save boundaries**. **`run.run_deck` (Slice 8d 2026-06-25):** a beacon-3 resume with the starter deck — while NodeMap reports earlier Combat beacons Resolved — is mechanically broken (player loses earned cards) in a way no surfaceable warning recovers. Joins by the membership criterion above. |

**On the rename:** `run.seed_map` → `run.session_core` is doc-only — the group name is not a serialised wire value; the per-DTO `SYSTEM_ID` consts (`"run.run_seed"`, `"run.node_map"`, `"run.run_deck"`) are unchanged. The new name codifies the membership criterion ("load-bearing structural state without which the run cannot resume coherently") so the group does not accrete into `run.seed_map_deck_offer_vehicle_…` by Slice 8g. Future additions must satisfy the criterion above or stay standalone with surfaceable defaults.

**How resume-atomicity is enforced:** the bootstrap site that hands `LoadResult` to the run-orchestrator (Slice 8b-3 / 8c / 8d: `SaveBootstrap.LoadAndInitialize` → `RunSceneHost.Initialize`) is the single decision point. The orchestrator inspects `LoadResult.Outcome` + `LoadResult.SkippedSystemIds` against the declared groups; if any group is not whole, the orchestrator regenerates the entire group from scratch instead of rehydrating partial state. There is no per-DTO "rehydrate-or-regenerate" toggle on individual `IRunStateSerializable` implementations — provenance is the bootstrap's concern, not the controller's (ADR-0011 #3 avoidance).

**Future groups must be declared in this ADR.** When the next coupled pair surfaces (likely `run.combat_state` once that DTO ships — a combat snapshot is meaningless without the matching deck it was drawn against), append a row to the table above OR add a member to an existing group if the criterion fits. No `[ResumeAtomic]` attribute, no central manifest file — drift-resistance is preserved the same way Decision 1's schema registry stays drift-resistant (one ADR row, CI-light enforcement via the bootstrap decision site).

**Unknown discriminator policy (Slice 8d Amendment 2026-06-25):** for polymorphic-payload DTOs (first instance: `RunDeckDto` with `CardEffectDto` subtypes dispatched by `effect_type` discriminator string), an unknown discriminator value at deserialisation throws `JsonSerializationException`, which the Slice 8b load path catches as a **DTO-scope** partial-skip per the partial-skip-at-the-entry-layer rule above. This is **not** an envelope-scope failure — sibling DTOs continue rehydrating; envelope checksum is not invalidated; the recovery chain does not advance to `.bak`. The DTO that owns the unknown discriminator regenerates (e.g., `RunDeck` → starter contents), and the resume-atomic group it participates in propagates the regenerate-together rule. Codified here so future polymorphic-payload DTOs (PendingCardOffer effects, future Mod payloads) follow the same policy without re-litigating.
...
- [ ] **(Slice 8c Amendment 2026-06-25)** `RunSceneHost.Initialize(LoadResult)` enforces the `run.session_core` (renamed from `run.seed_map` at Slice 8d) resume-atomic group: all members loaded ∧ none in `SkippedSystemIds` → rehydrate via internal `BeginRunFromLoaded(...)`; otherwise → fresh `BeginNewRun(null)`. Tests cover happy resume, mixed-skip both-or-neither fallback, and the determinism proof (`RunSeed_Resume_Preserves_Derived_Seeds`: same `DeriveCombatSeed(stepIndex)` post-resume as pre-save).
- [ ] **(Slice 8d Amendment 2026-06-25)** `RunDeckDto` ships in `WastelandRun.Save.Dtos` with `SYSTEM_ID = "run.run_deck"` + `SCHEMA_VERSION = 1`; nested `CardDefinitionDto` + polymorphic `CardEffectDto` (abstract base + 7 concrete subtypes — `WeaponAttackEffectDto`, `PlateEffectDto`, `RepairEffectDto`, `RepositionFlipEffectDto`, `RepositionToEffectDto`, `DrawEffectDto`, `BuffEffectDto` — each with hand-authored `EFFECT_TYPE_ID` const) + custom `CardEffectConverter` dispatching on `effect_type` discriminator string. Round-trip test covers wire shape per effect type. Unknown-discriminator test confirms `JsonSerializationException` at deserialisation → DTO-scope partial skip at the Slice 8b load path. `SchemaRegistry_Unique_test` passes with the new ID. `RunDeckSerializable` adapter mirrors `NodeMapSerializable` / `RunSeedSerializable` (snapshot-on-demand off `Func<RunDeck>`). `RunController.StartRun` takes a caller-supplied `RunDeck` (no internal `new RunDeck(...)` construction; no overload; no default param). `RunSceneHost.Initialize(LoadResult, NodeMapDto, RunSeedDto, RunDeckDto)` propagates `run.session_core` regenerate-together to deck. `RunDeck.Milestone1Starter()` stays a static factory (never default-ctor behaviour).
